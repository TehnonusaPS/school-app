...
<template>
  <div
    class="activity-card flex items-center gap-4 rounded-xl px-4 py-3 min-h-[60px] bg-muted/30 dark:bg-white/[0.04] border border-border/40 dark:border-white/[0.06] transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:bg-muted/50 dark:hover:bg-white/[0.07] cursor-pointer"
  >
    <!-- Leading: Date Circle -->
    <div
      v-if="isDateMode"
    ...
    >
      ...
      <span :class="['text-[9px] font-semibold leading-tight uppercase', styles.leadingText]">{{
        month
      }}</span>
    </div>
    ...
    <div
      ...
      :class="['flex items-center justify-center rounded-lg w-11 h-11 shrink-0', styles.iconBg]"
    >
      <component
        :is="leadingIcon"
        :class="['size-4', styles.iconColor]"
      />
    </div>

...
// Import API Services
import { fetchPublicEvents } from '@/services/academicCalendarService'
import { academicMonths, eventTypes, getEventBadgeStyle, getEventTypeInfo } from '../data/calendarConstants'
...
onMounted(async () => {
  ...
  try {
    ...
    // Fallback or derive academic year from dates if available
    if (events.value.length > 0) {
      // Derive active academic year name from first event date
      const dateParts = events.value[0].startDate.split('-')
      const year = parseInt(dateParts[0])
      const month = parseInt(dateParts[1])
      if (month >= 7) {
        academicYearName.value = `${year}/${year + 1}`
      } else {
        academicYearName.value = `${year - 1}/${year}`
      }

      // Default selected date to first event
      selectedDateStr.value = events.value[0].startDate
      const eventMonthVal = month - 1
      const matchedIdx = academicMonths.findIndex(m => m.val === eventMonthVal)
      if (matchedIdx !== -1) {
        activeMonthIdx.value = matchedIdx
      }
    } else {
      academicYearName.value = '2025/2026'
      const today = new Date()
      const currentMonthVal = today.getMonth()
      const matchedIdx = academicMonths.findIndex(m => m.val === currentMonthVal)
      activeMonthIdx.value = matchedIdx !== -1 ? matchedIdx : 0
    ...
    console.error('Error loading public calendar events:', err)
...
const currentCalendarYear = computed(() => {
  if (!academicYearName.value) return new Date().getFullYear()
  const startYear = parseInt(academicYearName.value.split('/')[0])
...
function getEventTypeBadgeClass(type) {
  return getEventBadgeStyle(type)
}
  ...
  >
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <PageHeader
        ...
        description="Detail agenda kegiatan sekolah, ujian semester, dan libur nasional aktif."
      />
      <Button
        variant="outline"
    ...
    <div v-if="isLoading" class="flex flex-col items-center justify-center py-20 space-y-4">
      ...
      <span class="text-xs font-bold text-muted-foreground">Memuat data agenda kalender...</span>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center bg-muted/30 p-4 rounded-2xl border border-border">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex flex-col gap-1 text-left">
            <span class="text-[9px] uppercase font-bold tracking-wider text-muted-foreground">Tahun Pelajaran Aktif</span>
            <span class="text-sm font-extrabold text-foreground">Tahun Ajaran {{ academicYearName }}</span>
          </div>
        </div>
                ...
                >
                  ...
                  <div class="flex items-start justify-between gap-2">
                    <div class="space-y-1">
                      ...
                      <p v-if="event.classroom_name" class="text-[9px] font-bold text-primary">Target: {{ event.classroom_name }}</p>
                    </div>
                  </div>
                ...
                >
                  <div class="space-y-0.5 overflow-hidden">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <span v-if="event.classroom_name && event.classroom_name !== 'Semua Kelas'" class="text-[8px] font-bold text-muted-foreground truncate">

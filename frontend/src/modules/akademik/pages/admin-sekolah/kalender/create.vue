...
const selectedTahun = ref('') // Dynamically set
const activeMonthIdx = ref(0) // Default to July (index 0) for new year planning
...
const availableTahunList = computed(() => {
  return tahunList.filter(t => {
    const startYear = parseInt(t.split('/')[0])
    const minDate = `${startYear}-07-01`
    const maxDate = `${startYear + 1}-06-30`
    const hasEvents = events.value.some(e => e.startDate >= minDate && e.startDate <= maxDate)
    const status = yearStatuses.value[t]?.status
    // Keep the year in the list if it is the currently selected year (even if events have been added during creation),
    // and if it hasn't been approved or submitted for approval.
    return (!hasEvents || t === selectedTahun.value) && status !== 'pending' && status !== 'approved'
  })
})

watch(selectedTahun, (newTahun) => {
  if (newTahun) {
    const startYear = parseInt(newTahun.split('/')[0])
    selectedDateStr.value = `${startYear}-07-01`
    activeMonthIdx.value = 0 // Reset to July when changing year
  }
})
...
onMounted(() => {
  ...
  if (availableTahunList.value.length > 0) {
    selectedTahun.value = availableTahunList.value[0]
  } else {
    selectedTahun.value = ''
  }
  ...
  if (selectedTahun.value) {
    ...
    selectedDateStr.value = '2027-07-01'
  }
...
const handleSelectCell = (cell) => {
  ...
  if (availableTahunList.value.length === 0) return
...
const selectedYearEventsCount = computed(() => {
  if (!selectedTahun.value) return 0
  const startYear = parseInt(selectedTahun.value.split('/')[0])
  const minDate = `${startYear}-07-01`
  const maxDate = `${startYear + 1}-06-30`
...
const handleSaveCalendar = () => {
  if (!selectedTahun.value) {
    toast.error('Gagal', { description: 'Silakan pilih Tahun Pelajaran terlebih dahulu.' })
    return
  }
  
  if (selectedYearEventsCount.value === 0) {
    toast.error('Gagal Membuat Kalender', {
      description: `Harap masukkan minimal 1 agenda kegiatan sebelum menyimpan kalender Tahun Pelajaran ${selectedTahun.value}.`
    })
    return
  }

  // Ensure status is explicitly saved as 'draft'
  const updatedStatuses = { ...yearStatuses.value }
  updatedStatuses[selectedTahun.value] = { status: 'draft', rejectedReason: '' }
  saveYearStatuses(updatedStatuses)
  
  toast.success('Kalender Dibuat', {
    description: `Draf Kalender Akademik Tahun Pelajaran ${selectedTahun.value} berhasil dibuat.`
  })
  router.push('/akademik/admin-sekolah/kalender')
}
</script>
  ...
  >
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <PageHeader
        ...
        description="Susun rancangan kegiatan sekolah baru per tanggal pada Tahun Pelajaran yang dipilih."
      />
    ...
    <!-- Warning Banner if no Year is available -->
    <div v-if="availableTahunList.length === 0" class="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-600 flex items-start gap-3">
      <AlertCircle class="h-5 w-5 shrink-0 mt-0.5" />
      <div class="text-xs font-bold leading-relaxed">
        Semua Tahun Pelajaran sudah terdaftar atau diajukan. Silakan hapus kalender tahun pelajaran yang ada di halaman utama jika ingin membuat ulang.
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center bg-muted/30 p-4 rounded-2xl border border-border">
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex flex-col gap-1 text-left">
          <span class="text-[9px] uppercase font-bold tracking-wider text-muted-foreground">Tahun Pelajaran Kalender Baru</span>
          <Select v-model="selectedTahun" :disabled="availableTahunList.length === 0">
            <SelectTrigger class="w-[220px] rounded-xl h-9 shadow-xs text-xs cursor-pointer bg-card">
              <SelectValue placeholder="Pilih Tahun Ajaran" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem v-for="t in availableTahunList" :key="t" :value="t">
                Tahun Ajaran {{ t }}
              </SelectItem>
            </SelectContent>
          </Select>
        ...
        <Card class="rounded-2xl border border-border bg-card shadow-xs overflow-hidden text-left">
          ...
          <CardContent class="p-4 space-y-4">
            ...
            <Button
              ...
              :disabled="availableTahunList.length === 0"
              @click="openAddDialog"
            >

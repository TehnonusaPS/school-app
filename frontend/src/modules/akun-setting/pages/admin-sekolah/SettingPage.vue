...
const form = ref({
  ...
  email: auth.user?.email || 'adminsekolah@mail.com'
})
...
<template>
  <div class="space-y-6 pb-12 text-left">
    <PageHeader
      ...
      description="Kelola informasi profil pribadi dan pengaturan keamanan akun Admin Sekolah Anda"
      :actions="actions"
    />
    ...
    <div class="grid gap-6 md:grid-cols-2">
      ...
      <!-- Row 1, Col 2: Identitas & Unit Kerja -->
      <div class="space-y-6">
        <!-- Identitas -->
        ...
        <!-- Informasi Sekolah -->
        <FormSection title="Informasi Unit Kerja" description="Instansi sekolah tempat Anda bertugas" :icon="BriefcaseBusiness">
          <div class="grid gap-4 md:grid-cols-2">
            <FormInput v-model="form.jabatan" label="Jabatan" disabled />
            <FormInput v-model="form.unitKerja" label="Nama Sekolah" disabled />
          </div>
        </FormSection>
      </div>

...
<template>
  <div class="space-y-12">
    <!-- 61. STAT CARD -->
    <section
      v-show="match('stat card statistik dashboard progress trend')"
      class="space-y-4"
    >
      <div class="border-b pb-2">
        <h2 class="text-2xl font-semibold text-primary">61. Stat Card</h2>
        <p class="text-sm text-muted-foreground mt-1">
          Kartu statistik responsif dengan desain Glassmorphism. Tersedia berbagai varian warna,
          indikator trend, dan progress bar.
        </p>
      </div>
    ...
    <!-- 62. WIDGET CARD -->
    <section
      v-show="match('widget card container dashboard wrapper')"
      class="space-y-4"
    >
      <div class="border-b pb-2">
        <h2 class="text-2xl font-semibold text-primary">62. Widget Card</h2>
        <p class="text-sm text-muted-foreground mt-1">
          Container generik untuk widget dashboard. Memiliki title, description, icon, dan slot
          untuk header action, konten, dan footer.
        </p>
      </div>
      <div class="grid gap-4 sm:grid-cols-2 p-6 border rounded-xl bg-card">
        ...
        >
          ...
          <div
            class="h-[120px] flex items-center justify-center text-sm text-muted-foreground bg-muted/20 rounded-lg border border-dashed"
          >
        ...
        <WidgetCard
          ...
          description="Pesan dan pengumuman terbaru"
          :icon="Bell"
        >
          <template #header-action>
            <Badge>3 Baru</Badge>
    ...
    <!-- 63. WIDGET LIST -->
    <section
      v-show="match('widget list daftar scrollable agenda pesan aktivitas')"
      class="space-y-4"
    >
      <div class="border-b pb-2">
        <h2 class="text-2xl font-semibold text-primary">63. Widget List</h2>
        <p class="text-sm text-muted-foreground mt-1">
          Widget daftar item dengan area scroll. Menggunakan scoped slot
          <code class="bg-muted px-1 rounded text-xs">#item</code> untuk custom layout setiap baris.
        </p>
      </div>
      <div class="grid gap-4 sm:grid-cols-2 p-6 border rounded-xl bg-card">
        <WidgetList
          title="Log Absensi"
          description="Rekap kehadiran hari ini"
          :icon="MessageCircle"
    ...
    <section
      v-show="match('widget progress list bar ranking indikator persentase')"
      class="space-y-4"
    >
      <div class="border-b pb-2">
        <h2 class="text-2xl font-semibold text-primary">64. Widget Progress List</h2>
        <p class="text-sm text-muted-foreground mt-1">
          Widget daftar dengan progress bar per item. Cocok untuk ranking, tingkat kehadiran, atau
      ...
      <div class="grid gap-4 sm:grid-cols-2 p-6 border rounded-xl bg-card">
        <WidgetProgressList
          ...
          description="Persentase bulan ini"
          :icon="Users"
    ...
    <!-- 65. DATA TABLE CARD -->
    <section
      v-show="match('data table tabel kartu toolbar pagination filter aksi')"
      class="space-y-4"
    >
      <div class="border-b pb-2">
        <h2 class="text-2xl font-semibold text-primary">65. Data Table Card</h2>
        <p class="text-sm text-muted-foreground mt-1">
          Komponen tabel data lengkap dengan toolbar (search & filter), konten tabel, dan
          pagination. Terdiri dari <code class="bg-muted px-1 rounded text-xs">DataTableCard</code>,
          <code class="bg-muted px-1 rounded text-xs">DataTableToolbar</code>,
          <code class="bg-muted px-1 rounded text-xs">BaseDataTable</code>, dan
          <code class="bg-muted px-1 rounded text-xs">DataTablePagination</code>.
        </p>
      </div>
    ...
    <!-- 67. FORM COMPONENTS -->
    <section
      v-show="match('form input select textarea tanggal date upload gambar section komponen')"
      class="space-y-4"
    >
      <div class="border-b pb-2">
        <h2 class="text-2xl font-semibold text-primary">67. Form Components</h2>
        <p class="text-sm text-muted-foreground mt-1">
          Kumpulan komponen form standar yang reusable:
          <code class="bg-muted px-1 rounded text-xs">FormInput</code>,
          <code class="bg-muted px-1 rounded text-xs">FormSelect</code>,
          <code class="bg-muted px-1 rounded text-xs">FormTextArea</code>,
          <code class="bg-muted px-1 rounded text-xs">FormDate</code>,
          <code class="bg-muted px-1 rounded text-xs">FormSection</code>, dan
          <code class="bg-muted px-1 rounded text-xs">ImageUpload</code>.
        </p>
      </div>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <FormSection
          title="FormInput"
          description="Field teks tunggal dengan label, placeholder, dan validasi error."
          :icon="Pencil"
        >
          <FormInput
            label="Nama Lengkap"
        ...
        <FormSection
          title="FormSelect"
          description="Dropdown pilihan dengan label dan opsi dinamis."
          :icon="ListFilter"
        >
          <FormSelect
            label="Kelas"
        ...
        <FormSection
          title="FormTextArea"
          description="Area teks multi-baris untuk keterangan atau catatan."
          :icon="AlignLeft"
        >
          <FormTextArea
            label="Catatan Siswa"
        ...
        <FormSection
          title="FormDate"
          description="Input tanggal dengan format standar."
          :icon="CalendarDays"
        >
          <FormDate
            label="Tanggal Lahir"
        ...
        <FormSection
          title="ImageUpload"
          description="Komponen upload gambar dengan preview, validasi error, dan catatan."
          :icon="Image"
        >
          <ImageUpload
        ...
        <FormSection
          title="FormSection"
          description="Container section form dengan icon, judul, dan deskripsi. Dapat menampung field apapun."
          :icon="ClipboardList"
        >
          <div
            class="text-sm text-muted-foreground bg-muted/30 rounded-lg border border-dashed p-8 text-center"
          >
      ...
      <div class="space-y-2">
        ...
        <div class="p-6 border rounded-xl bg-card space-y-6">
          <PageHeader
            ...
            description="Lengkapi semua informasi siswa di bawah ini."
          />
          <div class="grid gap-6 lg:grid-cols-2">
            <FormSection
    ...
    <!-- 68. ACTIVITY CARD -->
    <section
      v-show="match('activity card aktivitas agenda jadwal icon tanggal notifikasi')"
      class="space-y-4"
    >
      <div class="border-b pb-2">
        <h2 class="text-2xl font-semibold text-primary">68. Activity Card</h2>
        <p class="text-sm text-muted-foreground mt-1">
          Kartu item aktivitas/agenda dengan dua mode: <strong>mode tanggal</strong> (lingkaran
          dengan tanggal & bulan) dan <strong>mode icon</strong>. Tersedia 4 variant warna:
          <code class="bg-muted px-1 rounded text-xs">green</code>,
          <code class="bg-muted px-1 rounded text-xs">blue</code>,
          <code class="bg-muted px-1 rounded text-xs">purple</code>,
          <code class="bg-muted px-1 rounded text-xs">yellow</code>.
        </p>
      </div>
        ...
        <div class="space-y-2">
          ...
          <div class="space-y-2">
            <ActivityCard
              ...
              description="Aula Utama, pukul 09.00 WIB"
              variant="green"
            ...
            <ActivityCard
              ...
              description="Seluruh kelas, pukul 07.30 WIB"
              variant="blue"
              :trailingIcon="CalendarDays"
            />
            <ActivityCard
              ...
              description="Publikasi melalui aplikasi"
              variant="purple"
              :trailingIcon="Bell"
            />
            <ActivityCard
              ...
              description="Pertemuan orang tua wajib hadir"
              variant="yellow"
              :trailingIcon="School"
            />
          </div>
        ...
        <div class="space-y-2">
          ...
          <div class="space-y-2">
            <ActivityCard
              ...
              title="Ahmad Fauzi hadir tepat waktu"
              description="07:05 WIB — Kelas 10-A"
              variant="green"
            />
            <ActivityCard
              :leadingIcon="BookOpen"
              title="Materi Bab 5 diunggah"
              description="Matematika — Bpk. Rudi Hartono"
              variant="blue"
            />
            <ActivityCard
              :leadingIcon="Wallet"
              title="SPP bulan Juni lunas"
              description="Siti Rahayu — Rp 350.000"
              variant="purple"
            />
            <ActivityCard
              :leadingIcon="Bell"
              title="Peringatan: 3 siswa terlambat"
              description="Kelas 11-B — hari ini"
              variant="yellow"
            />
          </div>
        </div>

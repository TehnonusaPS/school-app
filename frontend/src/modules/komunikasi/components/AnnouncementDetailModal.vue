...
<template>
  <Dialog v-model:open="isOpen">
    <DialogContent
      class="sm:max-w-[550px] rounded-2xl border-border bg-card shadow-2xl max-h-[90vh] flex flex-col p-6"
    >
      <DialogHeader
        class="shrink-0"
        v-if="announcement"
      >
        <div class="flex flex-wrap items-center gap-2">
          <Badge
            :class="categoryBadgeClass(announcement.kategori)"
          ...
          <span
            class="text-xs text-foreground/80 font-bold bg-muted px-2 py-0.5 rounded border border-border/40"
            >{{ announcement.sekolah }}</span
          >
        </div>
        <DialogTitle
          class="text-xl font-extrabold text-foreground mt-3 leading-snug text-left select-text"
        >
          {{ announcement.judul }}
        </DialogTitle>
        <DialogDescription
          class="text-[11px] text-muted-foreground flex items-center gap-1.5 mt-2 font-mono font-medium"
        >
      ...
      <div
        class="flex-1 py-4 border-t border-b border-border/60 my-3 overflow-y-auto pr-1"
        v-if="announcement"
      >
      ...
      <DialogFooter class="shrink-0 flex justify-end gap-2 pt-2">
        <Button
          ...
          variant="outline"
          @click="isOpen = false"
          class="rounded-xl h-10 px-4 font-semibold"
        >
          Tutup
        </Button>
      </DialogFooter>

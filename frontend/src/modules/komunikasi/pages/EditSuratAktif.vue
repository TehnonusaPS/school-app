...
<template>
  ...
  >
    ...
    <PageHeader
      ...
      description="Perbarui formulir di bawah ini untuk mengedit surat keterangan siswa aktif."
      back
      @back="handleCancel"
    />
      ...
      <div class="p-6 sm:p-8 space-y-8 bg-slate-50/50 dark:bg-slate-900/10">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 w-full">
          ...
          <div class="space-y-3 sm:col-span-2">
            <label class="text-sm font-semibold text-foreground">Ganti Siswa (Opsional)</label>
            <Select @update:modelValue="onSiswaSelected" :disabled="isLoadingSiswa">
              <SelectTrigger class="w-full h-11 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm focus:ring-primary/30">
                <SelectValue :placeholder="isLoadingSiswa ? 'Memuat data siswa...' : 'Pilih Siswa lain jika ingin mengganti...'" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem 
            ...
            <p class="text-xs text-muted-foreground">Siswa saat ini: <span class="font-semibold">{{ form.nama }}</span></p>
          </div>
          ...
          <div class="space-y-3">
            ...
            <Input type="date" v-model="form.tanggalLahir" onclick="this.showPicker()" class="h-11 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm focus-visible:ring-primary/30 text-muted-foreground cursor-pointer" />
          </div>

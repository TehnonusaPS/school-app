...
// --- Role Verification ---
const isParent = computed(() => auth.user?.role === 'orang_tua')
...
function deleteFeedback(id) {
  if (confirm('Apakah Anda yakin ingin menghapus masukan ini?')) {
    ...
    toast.success('Feedback berhasil dihapus', {
      description: 'Aduan Anda telah ditarik dari papan evaluasi sekolah.'
    })
  }
}
...
// --- Principals/Admins & Shared Details State ---
...
<template>
  <div
    ...
    class="space-y-6 p-1"
  >
    <!-- 1. ORANG TUA / PARENT INTERFACE -->
    <div v-if="isParent" class="space-y-6">
    ...
    <!-- 2. KEPALA SEKOLAH / ADMIN INTERFACE -->
    ...
    <!-- Shared Details Modal Dialog (Visible to both roles) -->
    <FeedbackDetailModal 
      v-model:open="isDetailOpen" 
      :feedback="selectedFeedback" 
    />

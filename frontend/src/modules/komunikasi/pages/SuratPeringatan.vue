...
import { Plus, Mail, Eye, Edit, Trash2, Search, Calendar as CalendarIcon, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, AlertTriangle, CreditCard } from 'lucide-vue-next'
...
<template>
  <div class="space-y-6 p-1">
    ...
    <div>
      <div class="flex flex-col gap-1 mb-6">
        <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text">
          Daftar Surat Peringatan Kedisiplinan/Tunggakan
        </h1>
        <p class="text-xs sm:text-sm text-muted-foreground leading-relaxed">
          Kelola Surat Peringatan Kedisiplinan/Tunggakan
        </p>
      </div>
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-card border border-border/80 p-4 sm:p-5 rounded-2xl shadow-xs">
        ...
        <div class="flex gap-4">
          <!-- Total Keseluruhan -->
          <div class="flex items-center gap-3 sm:gap-4 border-2 border-primary/20 p-3 rounded-xl bg-primary/5 min-w-[140px]">
            <div class="shrink-0">
              <Mail class="size-5 sm:size-6 text-primary" />
            </div>
            <div>
              <p class="text-[10px] sm:text-[11px] font-semibold text-muted-foreground uppercase tracking-wider leading-none">Total Surat</p>
              <p class="text-xl sm:text-2xl font-extrabold text-foreground mt-1 tabular-nums leading-none">
                {{ store.items.length }}
              </p>
            </div>
          </div>
          ...
          <!-- Total Pelanggaran -->
          <div class="flex items-center gap-3 sm:gap-4 border-2 border-amber-500/20 p-3 rounded-xl bg-amber-500/5 min-w-[140px]">
            <div class="shrink-0">
              <AlertTriangle class="size-5 sm:size-6 text-amber-500" />
            </div>
            <div>
              <p class="text-[10px] sm:text-[11px] font-semibold text-muted-foreground uppercase tracking-wider leading-none">Kedisiplinan</p>
              <p class="text-xl sm:text-2xl font-extrabold text-foreground mt-1 tabular-nums leading-none">
                {{ totalPelanggaran }}
              </p>
            </div>
          ...
          <!-- Total Tunggakan -->
          <div class="flex items-center gap-3 sm:gap-4 border-2 border-rose-500/20 p-3 rounded-xl bg-rose-500/5 min-w-[140px]">
            <div class="shrink-0">
              <CreditCard class="size-5 sm:size-6 text-rose-500" />
            </div>
            <div>
              <p class="text-[10px] sm:text-[11px] font-semibold text-muted-foreground uppercase tracking-wider leading-none">Tunggakan</p>
              <p class="text-xl sm:text-2xl font-extrabold text-foreground mt-1 tabular-nums leading-none">
                {{ totalTunggakan }}
              </p>
            </div>
      ...
      <Table>
        <TableHeader class="bg-muted/30">
          <TableRow class="hover:bg-transparent">
            ...
            <TableHead class="font-semibold text-foreground">Jenis Surat</TableHead>
        ...
        <TableBody>
          <TableRow v-for="item in paginatedSuratList" :key="item.id" class="hover:bg-muted/30 transition-colors">
            ...
            <TableCell>
              <span class="font-medium" :class="item.jenisSurat === 'Surat Pelanggaran' ? 'text-amber-600' : 'text-rose-600'">
                {{ item.jenisSurat === 'Surat Pelanggaran' ? 'Kedisiplinan' : 'Tunggakan' }}
              </span>
            </TableCell>
            <TableCell>
    ...
    <!-- Print Preview Modal -->
    <SuratPeringatanPrintModal v-model:open="isPrintModalOpen" :data="selectedSurat" />
  </div>
</template>

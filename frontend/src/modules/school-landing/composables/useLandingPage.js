...
export function useLandingPage(slug) {
  ...
  const heroData = computed(() => ({
    title: data.value?.hero_title || dummyDataPublik.hero_title,
    subtitle: data.value?.hero_subtitle || dummyDataPublik.hero_subtitle,
    description: data.value?.hero_description || dummyDataPublik.hero_description,
    image: data.value?.hero_image || dummyDataPublik.hero_image,
    images: data.value?.hero_images?.length ? data.value.hero_images : dummyDataPublik.hero_images,
    ctaText: data.value?.hero_cta_text || dummyDataPublik.hero_cta_text,
    ctaLink: data.value?.hero_cta_link || dummyDataPublik.hero_cta_link
  }))

  const aboutData = computed(() => ({
    title: data.value?.about_title || dummyDataPublik.about_title,
    description: data.value?.about_description || dummyDataPublik.about_description,
    image: data.value?.about_image || dummyDataPublik.about_image,
    vision: data.value?.about_vision || dummyDataPublik.about_vision,
    mission: data.value?.about_mission?.length ? data.value.about_mission : dummyDataPublik.about_mission
  }))

  const brandingData = computed(() => ({
    primaryColor: data.value?.primary_color || dummyDataPublik.primary_color,
    secondaryColor: data.value?.secondary_color || dummyDataPublik.secondary_color,
    accentColor: data.value?.accent_color || dummyDataPublik.accent_color,
    logo: data.value?.logo || dummyDataPublik.logo
  }))

  // Helper untuk membersihkan iframe utuh menjadi url src murni
  ...
  const contactData = computed(() => ({
    email: data.value?.contact_email || dummyDataPublik.contact_email,
    phone: data.value?.contact_phone || dummyDataPublik.contact_phone,
    address: data.value?.contact_address || dummyDataPublik.contact_address,
    mapsEmbed: extractIframeSrc(data.value?.contact_maps_embed || dummyDataPublik.contact_maps_embed)
  }))

  const socialData = computed(() => ({
    instagram: data.value?.social_instagram || dummyDataPublik.social_instagram,
    facebook: data.value?.social_facebook || dummyDataPublik.social_facebook,
    youtube: data.value?.social_youtube || dummyDataPublik.social_youtube,
    tiktok: data.value?.social_tiktok || dummyDataPublik.social_tiktok
  }))

  const sections = computed(() => (data.value?.sections?.length ? data.value.sections : dummyDataPublik.sections))
  ...
  async function fetchData() {
    ...
    try {
      ...
      console.warn('Gagal memuat API landing, mencoba membaca data dari LocalStorage...')
      
      const foundations = JSON.parse(localStorage.getItem('superadmin_foundations') || '[]')
      const schools = JSON.parse(localStorage.getItem('superadmin_schools') || '[]')
      const allLocalData = [...foundations, ...schools]
      
      const localData = allLocalData.find(item => item.slug === slug)
      
      if (localData) {
        data.value = localData
      } else {
        console.warn('Data lokal tidak ditemukan berdasarkan slug, fallback ke data dummy.')
